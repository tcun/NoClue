...
func _enter_tree():
	if is_multiplayer_authority():
		...
		# Make player invisible to their own camera
		var visual_instance_objs: Array[VisualInstance3D] = []
		findByClass(self, "VisualInstance3D", visual_instance_objs)
		
		# Set layer masks for visual instances and camera culling
		for obj in visual_instance_objs:
			obj.set_layer_mask_value(1, false)  # Disable layer 1
			obj.set_layer_mask_value(2, true)   # Enable layer 2
		
		# Set camera to not cull layer 2
...
func _check_if_teleport_was_teleport_to_other_portal(to_position):
	...
	var transform_to_cur_portal = _portal_tracking_us.global_transform.affine_inverse()
	var position_rel_to_cur_portal = transform_to_cur_portal.xform(global_position)
	var to_position_offset = to_position - self.position
	var after_teleport_global_pos = global_position + to_position_offset
	var transform_to_other_portal = _portal_tracking_us.other_portal.global_transform.affine_inverse()
	var position_rel_to_other_portal = transform_to_other_portal.xform(after_teleport_global_pos)
	
	return position_rel_to_cur_portal.distance_to(position_rel_to_other_portal) < 1.0
...
func _snap_down_to_stairs_check():
	...
	if not is_on_floor() and velocity.y <= 0 and (_was_on_floor_last_frame or _snapped_to_stairs_last_frame) and $StairsBelowRayCast3D.is_colliding():
		...
		params.motion = Vector3(0, -0.5, 0)  # max_step_down is directly used here
...
func sync_position(new_pos: Vector3, new_rotation_y: float, new_velocity: Vector3):
